...
const { errors, Exception, sms, randomSetCodeGenerator } = require('utils');
...
class AuthService {
	...
	static async login(data) {
		...
		await session.withTransaction(async (session) => {
			...
			user.verificationCode = AuthService.verificationCode;
			// user.verificationCode = randomSetCodeGenerator(6);
	...
	static async refreshToken(data) {
		...
		await session.withTransaction(async (session) => {
			...
			if (
				moment(result.createdAt).add(jwt.refreshToken.expirationDuration, jwt.refreshToken.expirationUnit).isBefore(now)
			) {
			...
			const remaining =
				moment(result.createdAt).add(jwt.refreshToken.expirationDuration, jwt.refreshToken.expirationUnit) - now;
	...
	static async #getTokens(user, session) {
		...
		const refresh = await Session.findOneAndUpdate({ user: user.id }, { createdAt: new Date() }, { session });
